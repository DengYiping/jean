...
The General preferences pane exposes CodeRabbit CLI status, install, login, update, and managed-install deletion.
...
- CodeRabbit CLI review: `run_coderabbit_review`, which runs `coderabbit review --agent --dir <worktree>`.
- CodeRabbit PR review: `trigger_coderabbit_pr_review`, which posts `@coderabbitai review` on the linked PR.
