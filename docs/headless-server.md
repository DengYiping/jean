...
```bash
xvfb-run -a ./target/debug/jean-server --host 127.0.0.1 --port 3456
...
After `cargo build --manifest-path src-server/Cargo.toml --release` finishes,
`dist/` is no longer needed on the target server. Re-run `bun run build` before
compiling whenever frontend code changes.
