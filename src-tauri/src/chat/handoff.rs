...
pub(crate) fn latest_completed_backend(metadata: &SessionMetadata) -> Option<Backend> {
    metadata
        ...
        .and_then(|run| {
            ...
            if run.model.as_deref().is_some_and(crate::is_codex_model) {
...
mod tests {
    use super::*;
    use crate::chat::types::{RunEntry, SessionMetadata};
    ...
    fn formats_bounded_history_with_newest_context_preserved() {
        ...
        assert!(history.contains(TRUNCATED_HISTORY_MARKER));
    ...
    fn bounds_history_by_chars_and_handles_multibyte() {
        ...
        let many = (0..20)
            .map(|i| message(MessageRole::User, &format!("line {i} with unicode ❤"), i))
            .collect::<Vec<_>>();
        let bounded = format_handoff_history(&many, 60);
        assert!(bounded.chars().count() <= 60);
        assert!(bounded.contains(TRUNCATED_HISTORY_MARKER));
    }
