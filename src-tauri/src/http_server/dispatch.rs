...
pub async fn dispatch_command(
    ...
    match command {
        ...
        "update_session_state" => {
            ...
            let pending_codex_mcp_elicitations: Option<
                Vec<crate::chat::types::PendingCodexMcpElicitation>,
            > = field_opt(
                &args,
        ...
        "resolve_codex_update_command" => {
            let result = crate::codex_cli::resolve_codex_update_command(app.clone()).await?;
            to_value(result)
        }
        "get_claude_usage" => {
        ...
        "get_agent_browser_status" => {
            let result = crate::get_agent_browser_status(app.clone()).await?;
            to_value(result)
        }
        "ensure_agent_browser_profile" => {
            let result = crate::ensure_agent_browser_profile(app.clone()).await?;
            to_value(result)
        }
        "install_agent_browser" => {
            let result = crate::install_agent_browser(app.clone()).await?;
            emit_cache_invalidation(app, &["agent-browser"]);
            to_value(result)
        }
        "install_agent_browser_mcp" => {
            let backends: Option<Vec<String>> = from_field_opt(&args, "backends")?;
            let result = crate::install_agent_browser_mcp(app.clone(), backends).await?;
