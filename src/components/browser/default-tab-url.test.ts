...
import { resolveDefaultTabUrl, resolvePortUrl } from './default-tab-url'
...
describe('resolveDefaultTabUrl', () => {
  ...
  it('trims configured hosts', () => {
    expect(
      resolvePortUrl({ port: 5173, label: 'Vite', host: '  dev.local  ' })
    ).toBe('http://dev.local:5173')
  })
})
