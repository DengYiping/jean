...
export const ChatInput = memo(function ChatInput({
  ...
  // Context mention popover state (# issues, PRs, security, Linear)
  ...
  const handleChange = useCallback(
    (e: React.ChangeEvent<HTMLTextAreaElement>) => {
      ...
      if (!fileMentionOpen && !slashPopoverOpen) {
        if (prevChar === '#') {
      ...
      // Detect `$` skill and `/` command triggers (only if @/# popovers not open)
  ...
  const handlePaste = useCallback(
    async (e: React.ClipboardEvent) => {
      ...
      for (const item of items) {
        ...
        if (!(ALLOWED_IMAGE_TYPES as readonly string[]).includes(item.type)) {
  ...
  return (
    <div className="relative">
      ...
      <Textarea
        ...
        onPaste={handlePaste}
        disabled={false}
        className="min-h-[40px] max-h-[50vh] w-full resize-none overflow-x-hidden overflow-y-auto border-0 bg-transparent dark:bg-transparent p-0 font-mono text-base shadow-none focus-visible:ring-0 focus-visible:ring-offset-0 md:text-sm"
        rows={1}
        autoFocus={!isMobile}
      />
      {showHint && (
