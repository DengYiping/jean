...
describe('useChatWindowEvents worktree approval shortcuts', () => {
  ...
  function renderUseChatWindowEvents(
    ...
    const params: Parameters<typeof useChatWindowEvents>[0] = {
      ...
      handleInputNewSessionYolo: vi.fn(),
      handleInputNewSessionBuild: vi.fn(),
      handleInputNewWorktreeBuild: vi.fn(),
      handleInputNewWorktreeYolo: vi.fn(),
      handleToggleParallelExecutionPrompting: vi.fn(),
      scrollViewportRef,
  ...
  it('routes worktree yolo approval to the streaming handler while plan is still streaming', () => {
    const params = renderUseChatWindowEvents({
  ...
  it('routes approval shortcuts to the streaming handler while plan is still streaming', () => {
    const params = renderUseChatWindowEvents({
      hasStreamingPlan: true,
    })
    ...
    expect(params.handleStreamingPlanApproval).toHaveBeenCalledTimes(1)
