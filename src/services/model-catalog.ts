...
export function getCatalogModelPreferenceKey(
  catalog: ModelCatalog | null | undefined,
  backend: CliBackend,
  model: string
): string {
  const { baseModel } = getCatalogModelFastInfo(catalog, backend, model)
  return `${backend}:${baseModel}`
}

export function resolveRememberedCatalogFastModel(
  catalog: ModelCatalog | null | undefined,
  backend: CliBackend,
  model: string,
  rememberedFastModelKeys: readonly string[]
): string {
  const fastInfo = getCatalogModelFastInfo(catalog, backend, model)
  if (fastInfo.isFast || !fastInfo.supportsFast || !fastInfo.fastModel) {
    return model
  }

  return rememberedFastModelKeys.includes(
    getCatalogModelPreferenceKey(catalog, backend, model)
  )
    ? fastInfo.fastModel
    : model
}
