...
export const useTerminalStore = create<TerminalState>((set, get) => ({
  ...
  removeTerminal: (worktreeId, terminalId) =>
    set(state => {
      ...
      if (!hadTerminal && !wasRunning && !wasActive) return state
  ...
  registerStartedRun: (worktreeId, terminalId, command) =>
    set(state => {
      ...
      return {
        terminals: terminals.some(terminal => terminal.id === terminalId)
          ? state.terminals
          : {
              ...state.terminals,
              [worktreeId]: [
                ...terminals,
                {
                  ...
                  command,
                  commandArgs: null,
                  label: getFilename(command) || command,
                  kind: 'panel',
                },
              ],
            },
        ...
        runningTerminals,
        terminalVisible: true,
        terminalPanelOpen: { ...state.terminalPanelOpen, [worktreeId]: true },
      }
    }),
